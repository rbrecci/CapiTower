...
// Escolha carta contra habilidade. A opcao de carta sorteia 1 do catalogo da classe da run
// e pode repetir uma que ja esta no deck; a opcao de habilidade mostra o efeito real do proximo
// nivel da habilidade da classe (HABILIDADE_TEXTOS, portado de flet_mvp/capitower/content.py),
// aplicado de verdade pelo motor em core/combat.js.

import { MAX_NIVEL_HABILIDADE, HABILIDADE_TEXTOS, HABILIDADE_NOMES } from "../core/state.js";
...
export function renderRecompensa(container, run, cartaSorteada, onEscolherCarta, onEscolherHabilidade) {
  ...
  const nomeHabilidade = HABILIDADE_NOMES[run.classe];
  tituloHab.textContent = `Habilidade: ${nomeHabilidade} nivel ${run.nivelHabilidade} -> ${proximo}`;
  ...
  textoHab.textContent =
    run.nivelHabilidade >= MAX_NIVEL_HABILIDADE
      ? `Ja no nivel maximo de ${nomeHabilidade}.`
      : HABILIDADE_TEXTOS[run.classe][proximo];
